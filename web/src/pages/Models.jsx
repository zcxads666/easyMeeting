    ...
    <div className="pt-12 max-w-3xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-400 mt-1">Whisper 多种尺寸 · Qwen3-ASR，本地推理不依赖云端</p>
        </div>
        <span className="text-sm text-gray-400">占用 {formatBytes(diskUsage)}</span>
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          {models.filter((m) => m.kind === 'whisper').map((m) => (
            <ModelRow key={m.id} m={m} current={current} downloading={downloading} onDownload={download} onDelete={del} onSwitch={switchModel} />
          ))}
...
function ModelRow({ m, current, downloading, onDownload, onDelete, onSwitch }) {
    ...
    <div className="card p-5 flex items-center gap-4">
      <div className="flex-1">
        <div className="flex items-center gap-2">
          ...
          {isCurrent && <span className="text-xs px-2 py-0.5 rounded-full bg-green-100 text-green-600">当前使用</span>}
        </div>
      ...
      <div className="flex gap-2">
        {m.installed ? (
          ...
          <button className="btn-primary !py-1.5 text-sm" disabled={downloading === m.id} onClick={() => onDownload(m.id)}>

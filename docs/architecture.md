...
- **Node core**：会议、设置、TaskManager、Provider contract、受控音频 Range endpoint、RuntimeManager 和 diagnostics。
- **Python runtime**：设备检测、模型 lifecycle、faster-whisper/Transformers 推理及 Benchmark。

## Core services

- `TaskManager` 使用内存 lane：local=1、cloud=3、runtime=1。本地文件推理和 Benchmark 共享 local lane。
